...
export class PrestataireRegisterComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private router: Router, private ActivatedRoute: ActivatedRoute, private domSanitizer: DomSanitizer, private httpService: ServiceService) {
  ...
  RegisterNewPresta() {
    ...
    if (this.myForm.valid) {
      this.httpService.prestataireRegiste(this.myForm.value).subscribe(data => (this.successMessage = "Registration Success"), error => (this.successMessage = "Erreur "))
